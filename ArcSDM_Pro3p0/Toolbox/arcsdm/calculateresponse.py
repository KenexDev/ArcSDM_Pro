"""
    ...
    01/2018 Bug fixes for 10.x - Arianne Ford, Kenex Ltd.
    2019 Bug fixes for ArcMap 10.6 and 10.7 - Arianne Ford, Kenex Ltd.
...
def Execute(self, parameters, messages):
    ...
    try:
       ...
       # import arcsdm.missingdatavar_func;
        ...
        #    importlib.reload (arcsdm.missingdatavar_func);
        except :
            reload(arcsdm.sdmvalues);
            reload(arcsdm.workarounds_93);
            reload(arcsdm.missingdatavar_func);
    ...
    ##        Output_Raster = os.path.join(gp.ScratchWorkspace,Input_Raster[:11] + "_W")  
            ...
            if len(arcpy.ListFields(Input_Raster,"VAT_*"))>0:  
                arcpy.RemoveJoin_management(Input_Raster)
            ...
            gp.CopyRaster_management(Input_Raster,Temp_Raster,'#','#',NoDataArg2)
            gp.AddMessage(" Output_Raster: " + Output_Raster);
            if len(arcpy.ListFields(Input_Raster,"VAT_*"))>0:  
                arcpy.RemoveJoin_management(Input_Raster)
                arcpy.JoinField_management(Input_Raster,"VALUE",Wts_Table,"CLASS")
            else:  
                arcpy.JoinField_management(Input_Raster,"VALUE",Wts_Table,"CLASS")
            gp.Lookup_sa(Input_Raster,"WEIGHT",Output_Raster)
            #return;
    ...
    ##        gp.CreateRaster_sdm(Input_Raster, Wts_Table, "CLASS", "W_STD", Output_Raster, IgnoreMsgData, MissingDataValue)
            ...
            gp.JoinField_management(Input_Raster,"VALUE",Wts_Table,"CLASS")
            ...
            gp.JoinField_management(Input_Raster,"VALUE",Wts_Table,"CLASS")
            #gp.AddField_management(Input_Raster,"W_STD2","DOUBLE")
            #gp.CalculateField_management(Input_Raster,"W_STD2",Wts_Table+".W_STD")
            gp.Lookup_sa(Input_Raster,"W_STD",Output_Raster)
