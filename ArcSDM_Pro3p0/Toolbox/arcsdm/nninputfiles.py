...
def getSelectedRows( sites, extr_sites ):
    gcms = str(arcpy.GetCount_management(sites));
    gcmes = str(arcpy.GetCount_management(extr_sites));
    ...
    if gcms < gcmes:
        ...
        fids1 += ",".join(map(str,fidlist)) + ")"
        #print len(fidlist), fids
        return rowgen(arcpy.SearchCursor(extr_sites, fids1))
...
def getMinMaxValues( uc, evidence_names ):
    ...
    for evidence_name in evidence_names:
        minmaxdict[evidence_name] = {'minval':sys.maxsize, 'maxval':-sys.maxsize-1}
