...
def Execute(self, parameters, messages):
    ...
    ##Insert some field values in output table
        ...
        #for i in range(len(nSites)):
         #   effCumarea += 100.0 * effarea_rev.next() / totalarea
          #  effCumareaList.append(effCumarea)
           ...
           #TC Edit 28/02/2023 - changed .next() to .__next__
           
        for i in range(len(nSites)):
            effCumarea += 100.0 * effarea_rev.__next__() / totalarea
            effCumareaList.append(effCumarea)
            cumSites += 100.0 * nSites_rev.__next__() / totalsites
            cumSitesList.append(cumSites)
        ...
        while tblrow:
            ...
            tblrow.Eff_CumAre = effCumareaList_rev.__next__()
            Cum_Sites = cumSitesList_rev.__next__()
       ...
       # get the traceback object
        ...
        pymsg = "PYTHON ERRORS:\nTraceback Info:\n" + tbinfo + "\nError Info:\n    " + \
            str(type(sys.exc_info()[1]))+ ": " + str(type(sys.exc_info()[2])) + "\n"
