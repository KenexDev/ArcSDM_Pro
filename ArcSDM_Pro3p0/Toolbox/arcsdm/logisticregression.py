...
"""
    ...
    Updated by Arianne Ford, Kenex Ltd. 2018 - bug fixes for 10.x
...
def Execute(self, parameters, messages):
    ...
    try:
        ...
        #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            # Create an attribute table for RasterLayer
    ...
    #Check input files
        ...
        try:
            ...
            import subprocess
            #Tom updated 2 lines below on 10-08-23 using chatGPT. OLD CODE: p = subprocess.Popen([fnBat,params]).wait()
            cmd = [fnBat]
    ...
    ###ReadLRResults -------------------------------------------------------------------------------------------------------
        ...
        while strLine:
            ...
            if strLine.strip() == 'DATA':
                ...
                import math  # Import the math module to use NaN

                if len(lstLine) > 5:
                    # Initialize variables to store field values
                    id_value = int(lstLine[1].strip())
                    lr_post_prob_value = None
                    lr_std_dev_value = None
                    lrt_value = None

                    try:
                        lr_post_prob_value = float(lstLine[3].strip())
                    except ValueError:
                        lr_post_prob_value = math.nan  # Set to NaN if conversion fails

                    try:
                        lr_std_dev_value = float(lstLine[5].strip())
                    except ValueError:
                        lr_std_dev_value = math.nan  # Set to NaN if conversion fails

                    try:
                        lrt_value = float(lstLine[4].strip())
                    except ValueError:
                        lrt_value = math.nan  # Set to NaN if conversion fails

                    # Insert the row with the updated field values
                    vTabLRrow.SetValue("ID", id_value)
                    vTabLRrow.SetValue("LRPostProb", lr_post_prob_value)
                    vTabLRrow.SetValue("LR_Std_Dev", lr_std_dev_value)
                    vTabLRrow.SetValue("LRTValue", lrt_value)
